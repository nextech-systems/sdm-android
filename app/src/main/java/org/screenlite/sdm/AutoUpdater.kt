...
class AutoUpdater(private val context: Context) {
    ...
    suspend fun updateIfNeeded() = withContext(Dispatchers.IO) {
        ...
        // Read config from MDM managed configuration
        val managedConfig = ManagedConfig(context)

        val serverUrl = managedConfig.serverUrl
        val screenName = managedConfig.screenName

        Log.i(
            TAG,
            "Managed config — URL: $serverUrl, Screen: $screenName"
        )

        // Push config to web-kiosk only if installed
        if (installed) {

            val kioskConfigurator = KioskConfigurator(context)

            kioskConfigurator.pushServerUrl(serverUrl)

            if (screenName.isNotEmpty()) {
                kioskConfigurator.pushScreenName(screenName)
            }
            ...
            Log.w(
                TAG,
                "Web-kiosk not installed — config will be pushed after install"
            )
        }
    }
    ...
    private fun getInstalledVersionCode(packageName: String): Long {
        ...
        return try {
            ...
            Log.e(
                TAG,
                "Error getting version code for $packageName",
                e
            )
    ...
    suspend fun fetchLatestApkRelease(): ApkReleaseInfo? =
        withContext(Dispatchers.IO) {
            ...
            client.newCall(request).execute().use { response ->
                ...
                if (!response.isSuccessful) {
                    ...
                    Log.e(
                        TAG,
                        "GitHub API call failed: ${response.code}"
                    )
                ...
                for (i in 0 until assets.length()) {
                    ...
                    if (name.endsWith(".apk")) {
                        ...
                        Log.d(
                            TAG,
                            "Found APK asset: $name ($url)"
                        )

                        val apkInfo =
                            downloadAndExtractApkInfo(url)

                        if (apkInfo != null) {

                            return@withContext ApkReleaseInfo(
                                url,
                                apkInfo.first,
                                apkInfo.second
                            )
                            ...
                            Log.w(
                                TAG,
                                "Failed to extract version info from APK, falling back to tag name"
                            )
    ...
    private suspend fun downloadAndExtractApkInfo(
        apkUrl: String
    ): Pair<Long, String>? =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                client.newCall(
                    ...
                    if (!response.isSuccessful) {
                        ...
                        Log.e(
                            TAG,
                            "Failed to download APK for version check: ${response.code}"
                        )
                ...
                Log.e(
                    TAG,
                    "Error extracting version info from APK",
                    e
                )
    ...
    private fun extractVersionFromApk(
        apkFile: File
    ): Pair<Long, String>? {
    ...
    private fun getApkInfoFromPackageManager(
        apkFile: File
    ): Pair<Long, String>? {
        ...
        return try {
            ...
            Log.e(
                TAG,
                "Error getting package info from APK",
                e
            )
    ...
    suspend fun updateApp(apkUrl: String) =
        withContext(Dispatchers.IO) {
            ...
            client.newCall(
                ...
                if (!resp.isSuccessful) {
                    ...
                    Log.e(
                        TAG,
                        "Failed to download APK: ${resp.code}"
                    )
                ...
                Log.i(
                    TAG,
                    "APK downloaded successfully to ${apkFile.absolutePath}"
                )
            }
            ...
            val admin =
                ComponentName(
                    context,
                    ScreenliteDeviceAdminReceiver::class.java
                )
            ...
            Log.i(
                TAG,
                "Uninstall blocked for $kioskPackage"
            )
        }
    ...
    fun installApkSilently(
        ...
        val intent =
            Intent("org.screenlite.sdm.INSTALL_COMPLETE")
    ...
    private fun grantPermissions() {
        ...
        Log.i(
            TAG,
            "Granting permissions to $kioskPackage..."
        )
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            Log.w(
                TAG,
                "Skipping permission grant — unsupported Android version"
            )
        }
    }
}