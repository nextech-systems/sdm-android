...
class AutoUpdater(private val context: Context) {
    ...
    suspend fun updateIfNeeded() = withContext(Dispatchers.IO) {
        ...
        Log.d(TAG, "Web-kiosk installed: $installed")

        grantPermissions()

        val releaseInfo = fetchLatestApkRelease()
        if (releaseInfo == null) {
            Log.w(TAG, "Could not fetch latest release info — skipping update")
            return@withContext
        }

        if (!installed) {
            Log.i(TAG, "Web-kiosk not installed — installing version ${releaseInfo.versionName}")
            updateApp(releaseInfo.downloadUrl)
            return@withContext
        }

        val installedVersion = getInstalledVersionCode(kioskPackage)
        Log.i(TAG, "Installed: $installedVersion, Latest: ${releaseInfo.versionCode}")

        if (releaseInfo.versionCode > installedVersion) {
            Log.i(TAG, "Update available — downloading ${releaseInfo.versionName}")
            updateApp(releaseInfo.downloadUrl)
        } else {
            Log.i(TAG, "Web-kiosk is up to date")
        }
    }
    ...
    private fun getInstalledVersionCode(packageName: String): Long {
        return try {
            ...
            Log.w(TAG, "Could not get installed version code: ${e.message}")
            -1L
        }
    }
        ...
        client.newCall(request).execute().use { response ->
            ...
            for (i in 0 until assets.length()) {
                ...
                if (name.endsWith(".apk")) {
                    ...
                    return@withContext if (apkInfo != null) {
                        ApkReleaseInfo(url, apkInfo.first, apkInfo.second)
                    } else {
                        Log.w(TAG, "Falling back to tag name for version")
                        ApkReleaseInfo(url, parseVersionCode(tagName), tagName)
                    }
                }
            }
    ...
    private suspend fun downloadAndExtractApkInfo(apkUrl: String): Pair<Long, String>? =
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                extractVersionFromApk(tempApkFile)
            } catch (e: Exception) {
                Log.e(TAG, "Error extracting APK version info", e)
                null
            } finally {
        ...
        return try {
            ZipFile(apkFile).use { zip ->
                if (zip.getEntry("AndroidManifest.xml") != null) {
                    getApkInfoFromPackageManager(apkFile)
                } else {
    ...
    private fun getApkInfoFromPackageManager(apkFile: File): Pair<Long, String>? {
        return try {
            ...
            packageInfo?.let {
                val vc = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) it.longVersionCode
                else @Suppress("DEPRECATION") it.versionCode.toLong()
                Pair(vc, it.versionName ?: "unknown")
            }
        } catch (e: Exception) {
    ...
    private fun parseVersionCode(tagName: String): Long {
        val parts = tagName.trimStart('v', 'V').split(".", "-", "+")
        return try {
    ...
    suspend fun updateApp(apkUrl: String) = withContext(Dispatchers.IO) {
        ...
        client.newCall(Request.Builder().url(apkUrl).build()).execute().use { resp ->
            ...
            Log.i(TAG, "APK downloaded to ${apkFile.absolutePath}")
        }
